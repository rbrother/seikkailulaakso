...
namespace Net.Brotherus {
    ...
    public class AssetCreator {
        ...
        private static AssetCreator _instance = null;

        public static AssetCreator Instance {
            get {
                if (_instance == null) _instance = new AssetCreator();
                return _instance;
            }
        }

        private AssetCreator() {
            _effect = new BasicEffect(GameScreen.SharedGraphicsDevice);
        }

        private GraphicsDevice Device { get { return GameScreen.SharedGraphicsDevice; } }

        public Texture2D RectangleTile(int width, int height, int rot, Color color) {
            var vertices = PolygonTools.CreateRectangle(0.125f, 0.5f);
            PolygonShape shape = new PolygonShape(vertices, 20);
            return TextureFromShape(shape, MaterialType.Dots, color, 1f);
        }

        public Texture2D TextureFromFile(string fileName) {
        ...
        public void LoadContent(ContentManager contentManager) {
            _materials[MaterialType.Blank] = contentManager.Load<Texture2D>("SeikkailuLaaksoContent/Materials/blank");
            _materials[MaterialType.Dots] = contentManager.Load<Texture2D>("SeikkailuLaaksoContent/Materials/dots");
            _materials[MaterialType.Squares] = contentManager.Load<Texture2D>("SeikkailuLaaksoContent/Materials/squares");
            _materials[MaterialType.Waves] = contentManager.Load<Texture2D>("SeikkailuLaaksoContent/Materials/waves");
            _materials[MaterialType.Pavement] = contentManager.Load<Texture2D>("SeikkailuLaaksoContent/Materials/pavement");
        }
