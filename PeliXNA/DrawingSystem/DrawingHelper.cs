...
namespace FarseerGames.GettingStarted.DrawingSystem
{
    public static class DrawingHelper
    {
        ...
        public static Texture2D CreateCircleTexture(GraphicsDevice graphicsDevice, int radius, int borderWidth,
                                                    ...
                                                    Color color, Color borderColor)
        {
            // TODO: Replace this by drawing a circle with WPF and rendering it to file (can then also use some pattern)
            int y = -1;
            ...
            for (int i = 0; i < colors.Length; i++)
            {
                ...
                {
                    // Add some black lines to the color to see rotation angle
                    if (i % 4 != 0)
                    {
                        colors[i] = color;
                    }
