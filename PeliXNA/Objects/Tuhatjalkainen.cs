...
namespace Net.Brotherus
{
    public class Tuhatjalkainen
    {
        ...
        public Tuhatjalkainen(Vector2Fs position, GraphicsDevice graphicsDevice, PhysicsSimulator physicsSimulator)
        {
            ...
            {
                _legs.Add(new LegPair(physicsSimulator, graphicsDevice, _torso, new Vector2Fs(x, Leg.PART_SIZE.Y)));
            }
        }
    ...
    public class LegPair
    {
        ...
        public LegPair(PhysicsSimulator physicsSimulator, GraphicsDevice graphicsDevice, RectangleBodyGeom torso, Vector2Fs pos)
        {
    ...
    public class Leg
    {
        public static readonly Vector2Fs PART_SIZE = new Vector2Fs(80, 16);
        private PhysicsSimulator _physicsSimulator;
        ...
        public Leg(PhysicsSimulator physicsSimulator, GraphicsDevice graphicsDevice, RectangleBodyGeom torso,
            Vector2Fs attachPoint, float weight)
        {
            _physicsSimulator = physicsSimulator;

            _upper = new RectangleBodyGeom(torso.Position + attachPoint, PART_SIZE, weight, graphicsDevice, _physicsSimulator) { RotationDeg = 90 };
            _upperJoint = CreateJoint(torso, _upper, 0, 90, 180, attachPoint - new Vector2Fs(0, PART_SIZE.X * 0.5f));
            _lower = new RectangleBodyGeom(torso.Position + attachPoint + new Vector2Fs(0, PART_SIZE.X), PART_SIZE, weight, graphicsDevice, _physicsSimulator) { RotationDeg = 90 };
            _lowerJoint = CreateJoint(_upper, _lower, -135, 0, 135, new Vector2Fs(0, PART_SIZE.X * 0.5f));

        }

        private AngleTargetLimitJoint CreateJoint(RectangleBodyGeom a, RectangleBodyGeom b,
            double minAngleDegrees, double targetAngleDegrees, double maxAngleDegrees, Vector2Fs jointRelPosition)
        {
